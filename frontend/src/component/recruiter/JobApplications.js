...
const ApplicationTile = (props) => {
  ...
  const handleClose = () => {
    ...
    const candidateData = {
      ...
      education: application.jobApplicant.education
        ...
        .join(", "),
      skills: application.jobApplicant.skills,
      cv: `${server}${application.jobApplicant.cv}`,
      tenthCertificate: `${server}${application.jobApplicant.tenthCertificate}`,
      twelfthCertificate: `${server}${application.jobApplicant.twelfthCertificate}`,
      highestQualification:application.jobApplicant.highestQualification,
      profilepic:`${server}${application.jobApplicant.profile}`,
      gender:application.jobApplicant.gender,
      contactNumber:application.jobApplicant.contactNumber,
      dob: application.jobApplicant.dob,
      cgpa:application.jobApplicant.cgpa,
  ...
  const getCV = () => {
    if (
      application.jobApplicant.cv &&
      application.jobApplicant.cv !== ""
    ) {
      const address = `${server}${application.jobApplicant.cv}`;
      console.log(address);
      axios(address, {
        method: "GET",
        responseType: "blob",
  ...
  return (
    <Paper className={classes.jobTileOuter} elevation={3}>
      <Grid container>
        ...
        <Grid item>
            <Link to={`/candidate-details/${candidateData.name}`} onClick={handleViewDetails}>
